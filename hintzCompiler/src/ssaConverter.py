...
class SSAConverter():
        ...
        for v in currF.declaredVarsList:
            ...
            for stmtID in stmtIdsForDefsOfV:
                ...
                while len(W) != 0:      # while W != {} do:
                    X = W.pop()
                    """ TODO(kaakash): Need and API for this """
                    domFrontsOfX = self.dom_fronts.DFs[X]
    ...
    def renameVersionsOfVars(self):
        ...
        cfg = self.dom_fronts.doms.bbg.cfg
        updatedReadWriteAnalyzer = ReadWriteAnalyzer(self.dom_fronts.doms.bbg.cfg)
        ...
        for eachBB in self.dom_fronts.doms.dfs_preorder():
